...
- [x] Employee Records — `apps.hr.Employee`. **Updated**: ported additional fields from MiranErp's own `Employee` — `payment_method`/`bank_name`/`bank_account_number`/`bank_account_name` for payroll disbursement; `national_id`/`passport_number`/`date_of_birth`/`gender`/`marital_status`/`address`/`emergency_contact_name`/`emergency_contact_phone` for personal/statutory records; a self-referential `manager` FK for org hierarchy (`validate_manager` rejects self-assignment, data model only, not wired into approval routing); a `cost_center` FK linking to `apps.costcenters`. New "More details (bank & personal)" collapsible section plus Manager/Linked-account columns on the Employees table.
- [x] Visual Org Chart — new `/dashboard/hr/org-chart` page (designed fresh — neither MiranErp nor CoreERP had the UI, MiranErp only had the underlying `manager` data): builds a tree client-side from each `Employee.manager` field and renders it as nested boxes.
...
- [x] Employee Referral Program — `Applicant.referred_by` (nullable FK to `hr.Employee`, designed fresh — no MiranErp/CoreERP model to port), a traceable link only, not a rewards/payout system. Shown as a "Referred by" column plus an optional picker on the applicant form on `/dashboard/hr/recruitment`.
...
- [x] Fleet Management — new standalone `apps.fleet` app (designed fresh, Odoo-inspired — no MiranErp/CoreERP equivalent), reusing the `hr` permission module: `Vehicle` (registration/make/model/year/status) + `VehicleAssignment` (vehicle+employee+start/end date, at most one open assignment per vehicle enforced at the serializer level). Deliberately minimal — no maintenance/fuel-log system. New `/dashboard/hr/fleet` page.
...
- [x] Leave Management — `apps.hr.{LeaveType, LeaveRequest}`. Goes through `apps.approvals` as a fourth real consumer (after PurchaseOrder, PurchaseRequest, Expense), the identical Draft→Submitted→Approved/Rejected shape. Validates no two approved leave requests overlap for the same employee. **Updated**: approving a request that covers today now flips `Employee.status` to `on_leave` (a direct consequence of the decision itself, via `apps.hr.apps.HrConfig.ready`'s `on_decided` hook — no scheduled job needed for that part); a new `LeaveRequest.Status.CANCELLED` + `cancel` action reverts it back to `active` when it undoes an active approval — there's still no scheduler to flip status back automatically once a leave period simply ends, or forward once it starts if approved in advance, a known deliberate gap matching MiranErp's own documented one. `LeaveType.default_days_per_year` + `apps.hr.leave_balance.compute_leave_balance` (allocated/used/remaining, counting Submitted+Approved as used, ported from MiranErp's `leave_balance.py`) power a `/balances` endpoint shown on both the admin Leave & Contracts page and the self-service My Profile page. Further **updated**: `LeaveType` can now switch from a flat annual entitlement to a monthly accrual-to-date model (rate/month + one-year-lookback carryover cap, designed fresh — no MiranErp/Odoo model to port), and a new `apps.hr.PublicHoliday` calendar (also designed fresh) excludes any holiday falling inside a request's date range from `LeaveRequest.days`.
...
- [x] Performance Management — `apps.performance.PerformanceReview` (new app, shared with Training below): employee/reviewer/period/1-5 rating/comments, Draft while the reviewer edits it, `complete` action requires a rating and freezes it — the same two-state "book it, then it's final" shape `EmployeeContract` uses for facts-of-record, just without a GL posting side effect. Frontend at `/dashboard/hr/performance`. **Updated**: new `ReviewCycle` (designed fresh, no MiranErp/CoreERP model to port) groups several `PerformanceReview`s per employee/period from different raters (`rater_type`: Self/Manager/Peer/Other) into the 360° view a single review alone doesn't provide — `cycle` is nullable on `PerformanceReview`, so every existing standalone review keeps working unchanged. `ReviewCycle.average_rating` averages attached ratings, skipping unrated drafts.
